...
int BuscaImovelT(tImovel *imovel, char *tituloT, int tamanho)
{
    int i = 0;
    while(i<tamanho){
        if(imovel[i].a == 1 && imovel[i].i==1)
...
void ListarValor(tImovel *imovel, int tamanho)
{
    ...
    while(i<tamanho)
    {
        if((valor >= imovel[i].valor) && imovel[i].i==1)
        {
            if(imovel[i].c == 1)
            {
                printf("Imovel: %s\n", imovel[i].casa.titulo);
...
int main()
{
    ...
    system("color F0");
    if(LerArquivo(imovel))
    {
        printf("\t    Arquivo nao existe, por favor finalize o programa para cria-lo.\n");
        printf("\n\t    Para finalizar, pressione 9 no menu principal.\n"
               "\n\t  ||Clickar no X vermelho do programa NAO criara os arquivos||\n");
        printf("\n\t     Voce pode utilizar outras funcoes do programa, \n\t            depois criar os arquivos.\n");
    }
    if(!LerArquivo(imovel))
    {
        printf("\t\t     Todos os arquivos lidos com sucesso\n"
               "\n\t  Para salvar as alteracoes saia do programa pressionando 9\n"
               "\n\t||Clickar no X vermelho do programa NAO salvara nos arquivos||\n");
    }
    ...
    while(opmenu != 9)
    {
        ...
        if(opmenu == 3)
        {
            ...
            if(BuscaImovelT(imovel, tituloT, CalcularTamanhoArray(imovel)))
                printf("\nNao foi possivel encontrar o imovel\n\n");
        }
        if(opmenu == 4)
        {
            printf("\n\t\t1 - Casa\n\t\t2 - Apartamento\n\t\t3 - Terreno\n\t\t4 - Voltar ao menu\n");
